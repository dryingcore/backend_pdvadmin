import { pgTable, uuid, text, boolean, timestamp, uniqueIndex, numeric, date } from "drizzle-orm/pg-core"
...
export const eventos = pgTable("eventos", {
	...
	lucro: numeric({ precision: 10, scale:  2 }),
	status: text(),
	criadoEm: timestamp("criado_em", { mode: 'string' }).defaultNow().notNull(),
	atualizadoEm: timestamp("atualizado_em", { mode: 'string' }),
});
...
export const taxasEvento = pgTable("taxas_evento", {
	...
	antecipacao: numeric({ precision: 5, scale:  2 }).default('0').notNull(),
	criadoEm: timestamp("criado_em", { mode: 'string' }).defaultNow().notNull(),
}, (table) => [
...
export const transacoesDiarias = pgTable("transacoes_diarias", {
	...
	dinheiro: numeric({ precision: 10, scale:  2 }).default('0').notNull(),
	debito: numeric({ precision: 10, scale:  2 }).default('0').notNull(),
	credito: numeric({ precision: 10, scale:  2 }).default('0').notNull(),
	pix: numeric({ precision: 10, scale:  2 }).default('0').notNull(),
	status: text(),
	criadoEm: timestamp("criado_em", { mode: 'string' }).defaultNow().notNull(),
	atualizadoEm: timestamp("atualizado_em", { mode: 'string' }),
}, (table) => [
	uniqueIndex("unique_evento_loja_data").using("btree", table.eventoId.asc().nullsLast().op("date_ops"), table.lojaId.asc().nullsLast().op("uuid_ops"), table.dataTransacao.asc().nullsLast().op("date_ops")),
]);

export const fechamentosEvento = pgTable("fechamentos_evento", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	eventoId: uuid("evento_id").notNull(),
	valorTotalEvento: numeric("valor_total_evento", { precision: 12, scale:  2 }).notNull(),
	valorTotalComissionados: numeric("valor_total_comissionados", { precision: 12, scale:  2 }).notNull(),
	valorTotalPdvs: numeric("valor_total_pdvs", { precision: 12, scale:  2 }).notNull(),
	valorTotalLojas: numeric("valor_total_lojas", { precision: 12, scale:  2 }).notNull(),
	percentualStone: numeric("percentual_stone", { precision: 5, scale:  2 }).default('0.0').notNull(),
	valorStoneSobrePdvs: numeric("valor_stone_sobre_pdvs", { precision: 12, scale:  2 }).notNull(),
	lucroFinalEmpresa: numeric("lucro_final_empresa", { precision: 12, scale:  2 }).notNull(),
	criadoEm: timestamp("criado_em", { mode: 'string' }).defaultNow().notNull(),
	atualizadoEm: timestamp("atualizado_em", { mode: 'string' }),
}, (table) => [
